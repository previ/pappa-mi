  ...
  def post(self):
    ...
    if( self.request.get("cmd") == "copy" ):
      ...
      if isp:
        ...
        template_values = PostHandler.create_post(node=node.key, user=self.get_current_user(), title="Ispezione", content=isp.note, resources=[isp.key], attachments=isp.allegati)
  ...
  def post(self):
    ...
    if( preview ):
      ...
      if dieta:
        ...
        template_values = PostHandler.create_post(node=node.key, user=self.get_current_user(), title="Ispezione Diete speciali", content=dieta.note, resources=[dieta.key], attachments=dieta.allegati)

...
function onChannelMessage(a){a=jQuery.parseJSON(a.data);getNotificationsNum();$.pnotify({title:"Notifica",text:a.user+' ha aggiunto un <a href="'+a.source_uri+'">'+a.source_desc+'</a> su <a href="'+a.target_uri+'">'+a.target_desc+"</a>"})}
var tiny_mce_opts={script_url:"/js/tiny_mce/tiny_mce.js",width:"100%",content_css:"/js/tiny_mce/themes/advanced/skins/default/custom_content.css",theme_advanced_font_sizes:"10px,12px,13px,14px,16px,18px,20px",font_size_style_values:"10px,12px,13px,14px,16px,18px,20px",theme:"advanced",plugins:"autolink, autoresize",theme_advanced_buttons1:"bold,italic,underline,separator,strikethrough,bullist,numlist,undo,redo",theme_advanced_buttons2:"",theme_advanced_buttons3:"",theme_advanced_toolbar_location:"top",
theme_advanced_toolbar_align:"left",theme_advanced_resizing:!1};function onSuccess(a){"success"==a.response&&(a.url?window.location.href=a.url:$.pnotify({title:"Info",text:"Impostazioni salvate"}))}function onError(a,b,c){window.console&&console.log("textStatus: "+b+" errorThrown: "+c);window.location.href="/"}function floodUpdate(){time_left-=1;0==time_left&&$("#flood_error").modal("hide");message=time_left+" second";message=1==time_left?message+"o":message+"i";span.html(message)}
function onFloodError(a,b){$(".main").append(a.html);$("#flood_error").modal();$("#flood_error").on("hide",function(){clearInterval(floodErrorTimer);$("#flood_error").remove();b()});i=0;time_left=a.time;span=$(".seconds_left");time_left;floodUpdate();floodErrorTimer=setInterval(function(){floodUpdate()},980)}function getPostRootByElement(a){return a.parents(".s_post_root")}function getCommentRootByElement(a){return a.parents(".s_comment")}
...
function initPost(a){var b=a.find("form.s_new_comment_form");b.validate({ignore:"",errorClass:"error",errorPlacement:function(a,b){var e=$(b).parents(".form_parent");e.tooltip({title:a.text(),trigger:"manual"});e.tooltip("show")},highlight:function(a,b){$(a).parents(".form_parent").addClass(b)},unhighlight:function(a,b){$(a).parents(".form_parent").removeClass(b);$(a).tooltip("hide")},rules:{content:{required:!0,minlength:8}}});b.ajaxForm({clearForm:!0,dataType:"json",error:onError,success:function(b){"success"!=
b.response?"flooderror"==b.response?onFloodError(b,function(){}):onError():(a.find(".s_comment_list").append(b.html),a.find(".s_post_comment_num").text(b.num),a.find(".s_comment_submit").button("reset"),tinymce.get("comment_content_"+a.find(".s_post_root").attr("data-post-key")).setContent(""),b=a.find(".s_comment_list > li:last-child"),initComment(b))},beforeSubmit:function(){a.find(".s_comment_submit").button("loading")}});a.find(".s_post_comment").tinymce(tiny_mce_opts);a.find(".s_post_avatar").click(onAuthorDetail);
a.find(".post_sub").click(onPostSubscribe);a.find(".post_unsub").click(onPostUnsubscribe);a.find(".post_del").click(onPostDelete);a.find(".post_pin").click(onPostPin);a.find(".post_collapse").click(onPostCollapse);a.find(".post_edit").click(onPostEdit);a.find(".post_reshare").click(onPostReshare);a.find(".post_vote").click(onPostVote);a.find(".post_unvote").click(onPostVote);a.find(".s_post_votes_c").click(onVotesDetail);a.find(".s_post_reshares_c").click(onResharesDetail);a.find(".show_comment_form").click(onPostCommentFormExpand);
a.find(".show_comment_form").click(onPostCommentsExpand);a.find(".post_comments_expand").click(onPostCommentsExpand);initComment(a)}function initComment(a){a.find(".post_comment_del").click(onCommentDelete);a.find(".post_comment_edit").click(onCommentEdit);a.find(".post_comment_vote").click(onCommentVote);a.find(".post_comment_unvote").click(onCommentVote)}
...
c.tooltip({title:a.text(),trigger:"manual"});c.tooltip("show")},highlight:function(a,b){$(a).parents(".control-group").addClass(b)},unhighlight:function(a,b){$(a).parents(".control-group").removeClass(b);$(a).tooltip("hide")},rules:{content:{required:!0,minlength:8}}});a.ajaxForm({clearForm:!0,dataType:"html",error:onError,success:onEditSubmit,beforeSubmit:function(){b.find(".post_edit_submit").button("loading")}});a=$('<div class="s_edit_undo" style="display:none;"></div>');a.append(c.contents());
b.append(a);c.empty();c.append(d);d.find(".s_post_edit_content").tinymce(tiny_mce_opts);b.find(".s_post_commands").hide();d.show();d.find('input[name="attach_file"]').change(addAttach);d.find(".post_attach_delete").click(onAttachDelete);d.find(".post_edit_cancel").click(onEditCancel)})}})}
...
function onCommentEdit(){var a=getPostRootByElement($(this)),b=getCommentRootByElement($(this)),c=b.attr("data-comment-key"),a=a.find(".s_post_tools > .s_comment_edit_form").clone(),d=a.find("form");d.validate({ignore:"",errorClass:"error",errorPlacement:function(a,b){var c=$(b).parents(".control-group");c.tooltip({title:a.text(),trigger:"manual"});c.tooltip("show")},highlight:function(a,b){$(a).parents(".control-group").addClass(b)},unhighlight:function(a,b){$(a).parents(".control-group").removeClass(b);
$(a).tooltip("hide")},rules:{content:{required:!0,minlength:8}}});d.find('[name="comment"]').attr("value",c);d.ajaxForm({clearForm:!0,dataType:"json",error:onError,success:onCommentSubmit});a.find(".s_comment_edit_content").attr("value",$(this).parents(".s_post_container").find(".s_post_content").html());b.find(".s_edit_hollow").append(a);a.find(".s_comment_edit_content").tinymce(tiny_mce_opts);b.find(".s_post_commands").hide();b.find(".s_post_content").hide();a.find(".s_comment_edit_form").show();
a.show();a.find(".post_comment_cancel").click(onCommentEditCancel)}function onCommentSubmit(a){if("success"!=a.response&&"flooderror"==a.response)onFloodError(a,function(){});else{var b=$('[data-comment-key="'+a.comment+'"]');b.html(a.html);$(document).ready(function(){initComment(b)})}}var onCommentEditCancel=function(){var a=getCommentRootByElement($(this));a.find(".s_comment_edit_form").hide();a.find(".s_post_content").show();a.find(".s_post_commands").show();a.find(".s_edit_hollow").empty();return!1};
...
function onPostPin(){var a=getPostKeyByElement($(this)),b=$(this).attr("data-days");$.ajax({type:"POST",url:"/post/manage",data:{cmd:"pin_post",post:a,days:b},dataType:"json",error:onError,success:function(){$.pnotify({title:"Info",text:"Post evidenziato"})}})}
function onPostDelete(){var a=getPostKeyByElement($(this));confirm("Sei sicuro di voler cancellare il post?")&&$.ajax({type:"POST",url:"/post/manage",data:{cmd:"delete_open_post",post:a},dataType:"json",error:onError,success:function(){window.location.href="/stream"}})}
function onPostItemDelete(){var a=getPostKeyByElement($(this));confirm("Sei sicuro di voler cancellare il post?")&&$.ajax({type:"POST",url:"/post/manage",data:{cmd:"delete_open_post",post:a},dataType:"json",error:onError,success:function(){$('[data-post-key="'+a+'"]').remove()}})}
function onPostSubscribe(){var a={cmd:"subscribepost",post:getPostKeyByElement($(this))};$.ajax({url:"/post/subscribe",data:a,dataType:"json",error:onError,success:function(){$("#postsub").hide();$("#postunsub").show();$.pnotify({title:"Info",text:"Sei iscritto a questo post"})}})}
function onPostUnsubscribe(a){a=getPostKeyByElement($(this));$.ajax({url:"/post/subscribe",data:{cmd:"unsubscribepost",post:a},dataType:"json",error:onError,success:function(){$("#postsub").show();$("#postunsub").hide();$.pnotify({title:"Info",text:"Non sei pi\u00f9 iscritto a questo post"})}})}
function redraw_node(a){var b=a.attr("data-sub-status"),c=a.attr("data-ntfy-period");a.find(".ntfy_period > i").css("visibility","hidden");"true"==b?(a.find("#subscribe_btn").html("Disiscriviti"),a.find("#sub_prop").removeAttr("disabled"),a.find("#ntfy_period_"+c+" > i").css("visibility","visible")):(a.find("#sub_prop").attr("disabled","true"),a.find("#subscribe_btn").html("Iscriviti"))}
function onSubUnsubNode(){var a=$(this).parents(".node_root"),b=a.attr("data-node-key"),c=a.attr("data-sub-status");$.ajax({url:"/node/subscribe",data:{node:b,cmd:"true"==c?"unsubscribe":"subscribe"},dataType:"JSON",error:onError,success:function(b){a.attr("data-sub-status",b.subscribed);a.attr("data-ntfy-period",b.ntfy_period);onSuccess(b);$(document).ready(function(){redraw_node(a)})}})}
...
function init_search(){$("#search_text").typeahead({minLength:2,source:function(a){$("#search_node_list").empty();$("#loading").show();return $.post("/node/paginate",{query:a,cmd:"search_nodes"},function(a){$("#loading").hide();a=jQuery.parseJSON(a);a.html&&($("#search").tab("show"),$("#search_node_list").html(a.html))})}})}function onSearchPaginate(){var a=$(this).attr("data-offset");$('input[name="offset"]').attr("value",a);$("div.active > form").submit()}
...
function initNode(a){$("#new_post_form").validate({ignore:"",errorClass:"error",errorPlacement:function(a,c){var d=$(c).parents(".control-group");d.tooltip({title:a.text(),trigger:"manual"});d.tooltip("show")},highlight:function(a,c){$(a).parents(".control-group").addClass(c)},unhighlight:function(a,c){$(a).parents(".control-group").removeClass(c);$(a).tooltip("hide")},rules:{content:{required:!0,minlength:8}}});$("#post_content_text").tinymce(tiny_mce_opts);$("#new_post_form").ajaxForm({clearForm:!0,
dataType:"html",error:onError,success:function(a){$("#main_stream_list").prepend(a);$("#open_post_submit").button("reset");a=$("#main_stream_list li:first-child");initPostList(a);tinymce.get("post_content_text").setContent("")},beforeSubmit:function(){$("#open_post_submit").button("loading");$("#new_post").slideUp()}});$("#subscribe_btn").on("click",onSubUnsubNode);$(".ntfy_period").on("click",onNotificationPeriod);$("#more_posts").click(onMoreClick);"all"!=a&&"news"!=a&&redraw_node($(".node_root"));
$('input[name="attach_file"]').change(addAttach);$(".post_attach_delete").click(onAttachDelete)}function init_ntfy(){var a=$("#ntf_cnt");a.attr("data-visible","false");a.popover({html:!0,trigger:"manual"}).click(function(a){onOpenNotifications(a)});$(document).click(function(){$('[data-visible="true"].s_popover').each(function(){$(this).popover("hide");$(this).attr("data-visible","false")})})}
...
function onOpenNotifications(a){var b={cmd:"ntfy_summary"};"false"==$("#ntf_cnt").attr("data-visible")&&$.ajax({type:"POST",url:"/ntfctn/paginate",data:b,dataType:"json",error:onError,success:function(b){var d=$("#ntf_cnt");d.attr("data-content",b.html);d.popover("show");d.attr("data-visible","true");$(".s_glob_ntfy").text("0");$(".s_glob_ntfy").removeClass("badge-warning");a.preventDefault()}})}function addAttach(){var a=$(this);a.clone().appendTo(a.parent())}
function onMoreClick(){var a=$(this).parents(".node_root").attr("data-node-key");loadPosts(a,$("#main_stream").attr("next_cursor"))}function loadNode(a,b){$.ajax({type:"POST",url:"/node/paginate",data:{cmd:"node_main",node:a,node_name:b},dataType:"json",error:onError,success:function(b){"success"==b.response&&($("#node_container").empty(),$("#node_container").append(b.html),$(document).ready(function(){initNode(a);loadPosts(a,"")}))}})}
function loadPosts(a,b){$("#main_stream_list").append('<li class="s_loading"><span class="offset3 span2">Loading... <img src="/img/loading.gif"></span></li>');$.ajax({type:"POST",url:"/post/paginate",data:{cmd:"post_main",node:a,cursor:b},dataType:"json",error:onError,success:function(b){$("#form_node").attr("value",a);$("#main_stream").attr("next_cursor",b.cursor);"success"==b.response&&($("#main_stream_list").find("li.s_loading").remove(),$("#main_stream_list").append(b.html),initPostList($("#main_stream_list")),
...
function onResharesDetail(){var a={cmd:"reshare_list",post:getPostKeyByElement($(this))};$.ajax({type:"POST",url:"//post/manage/managepost",data:a,dataType:"json",error:onError,success:function(a){$("body").append(a.html);$("#reshares_modal").modal()}})}
function onAuthorDetail(){var a=$(this),b={cmd:"author_detail",post:getPostKeyByElement($(this))};$.ajax({type:"POST",url:"/post/manage",data:b,dataType:"json",error:onError,success:function(b){a.attr("data-content",b.html);a.popover("show");a.attr("data-visible","true")}})}
...
function initForm(){ulmsg=!0;$("#new-data-form").find("[data-content]").popover({delay:{show:500,hide:1E3},title:"Informazioni"});$("#new-data").find("[type='radio']").button()}var auto_open_nc=!1;jQuery.validator.addMethod("numpastibambini",function(a){return parseInt(a)<=parseInt($("#e_numeroPastiTotale").val())});jQuery.validator.addMethod("numpastispeciali",function(a){return a<=parseInt($("#e_numeroPastiBambini").val())});
function opennewwiz(a){$("#new-data-form").load(a,function(){initForm();$("#form0").formwizard({formPluginEnabled:!0,validationEnabled:!0,validationOptions:{errorClass:"error",errorPlacement:function(a,c){var d=$(c).parents(".control-group");d.tooltip({title:a.text(),trigger:"manual"});d.tooltip("show")},highlight:function(a,c){$(a).parents(".control-group").addClass(c)},unhighlight:function(a,c){var d=$(a).parents(".control-group");d.removeClass(c);d.tooltip("hide")}},focusFirstInput:!0,textNext:"Avanti",
textBack:"Indietro",disableUIStyles:!0,remoteAjax:{step1:{url:a+"val",dataType:"text",success:function(a){if("Ok"!=a)return $("#formwiz-container").html('<div id="formwiz-error" class="alert alert-error fade in" style="display:none;"><a class="close" data-dismiss="alert" href="#">&times;</a>'+a+"</div>"),$("#formwiz-error").show(),$("#formwiz-error").alert(),!1;$("#formwiz-error").alert("close");return!0}}},formOptions:{success:function(a){$("#new-data-preview").html(a);$("#form-error").html()?($("#form-error").dialog({modal:!0,
width:"40em",zIndex:3,autoOpen:!1,buttons:[{text:"Ok",click:function(){$("#form-error").detach();$(this).dialog("close");$("#e_submit").button("reset")}}]}),$("#form-error").dialog("open"),$("#new-data-preview").html("")):($("#new-data-form").hide(),$("#new-data-preview").show(),$("#form1").find("#node").val($("#form_node").val()),$("#activity_list li:first-child").attr("id")&&$("#form1").find("#act_last").val($("#activity_list li:first-child").attr("id").substring(9)),$("#form1").ajaxForm({error:onError,
...
function opennewdieta(){$("#new-data-form").html("");$("#new-data-preview").html("");opennewwiz("/isp/dieta")}function opennewnota(){$("#new-data-form").html("");$("#new-data-preview").html("");opennewitem("/isp/nota")}function previewback(){$("#e_submit").button("reset");$("#new-data-preview").hide();$("#new-data-form").show()};var infowindow,locations={},list,xml=null,map,iLast,locArray,offset=0,markersArray=[],lat,lon,image=new google.maps.MarkerImage("http://google-maps-icons.googlecode.com/files/school.png",new google.maps.Size(32,37),new google.maps.Point(0,0),new google.maps.Point(16,37)),shadow=new google.maps.MarkerImage("http://google-maps-icons.googlecode.com/files/shadow.png",new google.maps.Size(51,37),new google.maps.Point(0,0),new google.maps.Point(16,37)),fluster=null,fstyles={"0":{image:"http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m1.png",
textColor:"#FFFFFF",width:53,height:52},10:{image:"http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m2.png",textColor:"#FFFFFF",width:56,height:55},20:{image:"http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m3.png",textColor:"#FFFFFF",width:66,height:65}};
function drawMap(a){var b=$("#e_citta").val(),c=$("#e_z").val(),d=$("#e_cc").val(),e=$("#e_tipo").val(),f=$("#e_numcm").val();jmarkers=jQuery(a);jmarker=jmarkers.find("marker");jmarker.each(function(){var a=jQuery(this),h=a.attr("key"),j=a.attr("nome"),k=a.attr("indirizzo"),g=a.attr("tipo"),l=a.attr("numcm"),m=a.attr("citta"),n=a.attr("zona"),p=a.attr("cc");if((!b||m==b)&&(!c||n==c)&&(!d||p==d)&&(!e||g==e)&&(0==f||0<l))a=new google.maps.LatLng(parseFloat(a.attr("lat")),parseFloat(a.attr("lon"))),
createMarker({key:h,latlng:a,name:j,address:k,type:g})});jmarkers.find("markers").attr("cur")?(offset+=1,jQuery.get("/map?cmd=all&cur="+jmarkers.find("markers").attr("cur")+"&offset="+offset,{},drawMap)):($("#loading").hide(),fluster.initialize())}
